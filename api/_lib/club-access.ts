...
export async function accessibleStores(session: Pick<AccountRow,'id'|'role'|'club_id'>, sql: ReturnType<typeof getSql>) {
  ...
  return sql`SELECT account_id,club_id,area,data FROM club_stores WHERE (${session.role==='superadmin'} OR club_id=${session.club_id}) AND club_id IN (SELECT id FROM clubs WHERE activo=TRUE) AND area IN ('team','rivals')`;
}
export async function accessibleAccounts(session: Pick<AccountRow,'role'|'club_id'> | null, sql: ReturnType<typeof getSql>) {
  ...
  if(session.role==='superadmin') return sql`SELECT * FROM club_accounts WHERE role='superadmin' OR club_id IN (SELECT id FROM clubs WHERE activo=TRUE) ORDER BY created_at`;
