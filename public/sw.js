const CACHE = 'convocatria-shell-v16';
const SHELL = ['/', '/manifest.webmanifest', '/convocatria.png', '/escudo-ud-oliva.jpg'];
self.addEventListener('install', (event) => { event.waitUntil(caches.open(CACHE).then((cache) => cache.addAll(SHELL))); self.skipWaiting(); });
self.addEventListener('activate', (event) => { event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => (key.startsWith('convo-shell-') || key.startsWith('convocatria-shell-')) && key !== CACHE).map((key) => caches.delete(key))))); self.clients.claim(); });
