...
const aiSessionToTrainingSession = (session: TrainingAISession): TrainingSession => ({
  ...
  blocks:session.exercises.map((exercise,index)=>({id:`ai-block-${index}-${exercise.id}`,title:exercise.block,minutes:exercise.duration,task:'',exercises:[{id:exercise.id,instanceId:exercise.id,minutes:exercise.duration,title:exercise.name,taskType:exercise.objective,description:exercise.development,organization:exercise.organization,coaching:exercise.coachingPoints.join(' · '),ageBenefit:'Sesión generada y revisada para este equipo.',board:exercise.board.map(piece=>({...piece})),actions:exercise.actions.map(action=>({...action,from:{...action.from},to:{...action.to}})),steps:exercise.instructions,minPlayers:exercise.players,maxPlayers:exercise.players,objective:exercise.objective,space:exercise.space,materials:exercise.material,variants:exercise.variants,coachingPoints:exercise.coachingPoints}] }))
});
