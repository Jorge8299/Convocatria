...
const statsMessage = (match: MatchStat, team: TeamData) => {
  let text = `*⚽ ${team.name} ${match.ourScore} - ${match.rivalScore} ${match.rival}*\n📅 ${formatDate(match.date)}\n\n*ESTADÍSTICAS*\n`;
...
function StatsView({ team, rivales, onSave }: { team: TeamData; rivales: Rival[]; onSave: (match: MatchStat) => void }) {
  ...
  return <div className="stats-layout"><section className="form-card"><div className="form-card-header"><div><span>1</span><h2>Partido</h2></div></div><div className="form-card-body"><div className="field-grid"><Field label="Rival"><div className="select-wrap"><select value={meta.rival} onChange={(event) => setMeta((current) => ({ ...current, rival: event.target.value }))}><option value="">Selecciona un rival</option>{rivales.map((rival) => <option key={rival.id} value={rival.nombre}>{rival.nombre}</option>)}</select><ChevronDown size={18} /></div></Field><Field label="Fecha"><input type="date" value={meta.date} onChange={(event) => setMeta((current) => ({ ...current, date: event.target.value }))} /></Field></div><div className="segmented compact"><button className={meta.home ? 'active' : ''} onClick={() => setMeta((current) => ({ ...current, home: true }))}><Home size={17} /> En casa</button><button className={!meta.home ? 'active' : ''} onClick={() => setMeta((current) => ({ ...current, home: false }))}><Plane size={17} /> Fuera</button></div><div className="score-input"><div><span>{team.name}</span><NumberControl label={`Goles de ${team.name}`} value={meta.ourScore} onChange={(ourScore) => setMeta((current) => ({ ...current, ourScore }))} /></div><strong>—</strong><div><span>{meta.rival || 'Rival'}</span><NumberControl label={`Goles de ${meta.rival || 'Rival'}`} value={meta.rivalScore} onChange={(rivalScore) => setMeta((current) => ({ ...current, rivalScore }))} /></div></div><Field label="Observaciones generales"><textarea rows={2} value={meta.notes} onChange={(event) => setMeta((current) => ({ ...current, notes: event.target.value }))} placeholder="Opcional" /></Field></div></section>
...
function SavedMatchEditor({ match, team, rivales, onCancel, onSave }: { match: MatchStat; team: TeamData; rivales: Rival[]; onCancel: () => void; onSave: (match: MatchStat) => void }) {
  ...
  return <div className="saved-match-editor">
    <div className="editor-heading"><button className="text-button" onClick={onCancel}><ArrowLeft size={17} /> Volver a partidos</button><div><span className="eyebrow">EDITANDO PARTIDO</span><h2>{team.name} vs {draft.rival}</h2><p>Los cambios sustituirán las estadísticas guardadas.</p></div></div>
    <section className="form-card"><div className="form-card-header"><div><span>1</span><h2>Datos del partido</h2></div></div><div className="form-card-body"><div className="field-grid"><Field label="Rival"><div className="select-wrap"><select value={draft.rival} onChange={(event) => setDraft((current) => ({ ...current, rival: event.target.value }))}><option value="">Selecciona un rival</option>{rivalOptions.map((rival) => <option key={rival.id} value={rival.nombre}>{rival.nombre}</option>)}</select><ChevronDown size={18} /></div></Field><Field label="Fecha"><input type="date" value={draft.date} onChange={(event) => setDraft((current) => ({ ...current, date: event.target.value }))} /></Field></div><div className="segmented compact"><button className={draft.home ? 'active' : ''} onClick={() => setDraft((current) => ({ ...current, home: true }))}><Home size={17} /> En casa</button><button className={!draft.home ? 'active' : ''} onClick={() => setDraft((current) => ({ ...current, home: false }))}><Plane size={17} /> Fuera</button></div><div className="score-input"><div><span>{team.name}</span><NumberControl label={`Goles de ${team.name}`} value={draft.ourScore} onChange={(ourScore) => setDraft((current) => ({ ...current, ourScore }))} /></div><strong>—</strong><div><span>{draft.rival || 'Rival'}</span><NumberControl label={`Goles de ${draft.rival || 'Rival'}`} value={draft.rivalScore} onChange={(rivalScore) => setDraft((current) => ({ ...current, rivalScore }))} /></div></div><Field label="Observaciones generales"><textarea rows={3} value={draft.notes} onChange={(event) => setDraft((current) => ({ ...current, notes: event.target.value }))} placeholder="Opcional" /></Field></div></section>
